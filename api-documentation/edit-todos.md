## Edit Todos API Documentation

### Request

`POST https://localhost/api/[id]/todo/edit`
...
| todoName | Name of todo to be changed to, required, max length of 255 characters, only regex `[[a-zA-Z0-9._ \t]` characters allowed |
...
|-|-|
| 201 | Successful edit todos request |
| 400 | Unsuccessful request, request parameters does not fit requirements above |
| 401 | Unsuccessful request, JWT token invalid |
| 403 | Unsuccessful request, not authorised to request change of todos controlled by another user |
