## Edit Account API Documentation

### Request

`POST https://localhost/api/[id]/account/edit`
...
| username | Username of user to be changed to, required, max length of 255 characters, only regex `[[a-zA-Z0-9._ \t]` characters allowed |
| address | Address of user to be changed to, required, date must exist and be in `YYYY-MM-DD` format |
| image | Profile picture of user to be changed to |

### Response

| Response Status | Response Meaning |
|-|-|
| 201 | Successful edit account request |
| 400 | Unsuccessful request, request parameters does not fit requirements above |
| 401 | Unsuccessful request, JWT token invalid |
| 403 | Unsuccessful request, not authorised to request change of account data of another user |
