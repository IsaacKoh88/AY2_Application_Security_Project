## Get Total Expense API Documentation

### Request

`POST https://localhost/api/[id]/expense/historyOrder`
...
| Parameter Type | Requirements |
|----------------|--------------|
| OrderBy | Response order, required, must be an accepted order |
| ID | UUID of user, required, only regex `[0-9A-F]{8}-[0-9A-F]{4}-4[0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}` format allowed |
| Month | Queried month, required, month must exist and be in `YYYY-MM` format |

### Response

| Response Status | Response Meaning |
|-|-|
| 200 | Successful request, returns order history data of queried user |
